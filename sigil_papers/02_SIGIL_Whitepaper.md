...
SIGIL's security posture was not achieved in one pass. It was built through five rounds of independent review, each of which found real problems, and each of which was addressed completely before the next round began.

The first round (v1.1.0) was the hardest, not technically, but philosophically. Two independent red team assessments found 53 cases where the code contradicted its own promises: error messages that weren't true, parameters that didn't work, API keys leaked into audit logs. All 53 were fixed, and the README was rewritten to stop overclaiming.
...
SIGIL exists as an existence proof that the baseline is achievable by a single developer working alone with open-source tools. The implementation is approximately 3,000 lines of auditable Python with a 310-test suite, hardened across five rounds of security review totaling 88 findings, all remediated. Any organization shipping agentic systems at scale has no excuse for doing less.
