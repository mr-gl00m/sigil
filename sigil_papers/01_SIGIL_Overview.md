...
On April 7, 2026, Anthropic released Mythos -- a model that found 27-year-old vulnerabilities in hardened systems, in hours, for pocket change. This means:
...
SIGIL has been hardened across five rounds of independent security review. Here's what matters about that history: the first round found that SIGIL's own code was lying, error messages that weren't true, parameters that didn't work, methods that did nothing when called. Every one of those findings was fixed, and the documentation was rewritten to stop overclaiming. That's the standard the project holds itself to: when someone finds a problem, it gets fixed, not explained away.

| Round | What was found | Result |
|-------|---------------|--------|
| Initial audit (v1.1.0) | 53 findings -- code honesty, security hazards, robustness | All remediated |
...
**Total: 88 findings identified. 88 remediated.** Zero outstanding.
